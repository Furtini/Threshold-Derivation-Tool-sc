...
public class GenerateTablePerMethod {
    ...
    private void geraListaPreOrdenacao(ArrayList<Metrics> listMetrics) {
        listaPreOrdem = new ArrayList<Value>();
    ...
    private ArrayList<Integer> getQtdMetodosRepetidos() {
        ArrayList<Integer> qtdDeMetodosRepetidos = new ArrayList<Integer>();
        String auxc = "-1";
        int ic = 0, cont = 0, tam = listaPreOrdem.size();
        for (Value x : listaPreOrdem) {
            if (!(x.getPackagi() + ":" + x.getSource() + ":" + x.getName()).equalsIgnoreCase(auxc) && !auxc.equalsIgnoreCase("-1")) {
            ...
            auxc = x.getPackagi() + ":" + x.getSource() + ":" + x.getName();
        }
    ...
    private ArrayList<Value> getListaAgrupada() {
        ...
        ArrayList<Value> agrupado = new ArrayList<Value>();
        Value auxv = new Value("-1", "", "-1", "");
        auxv.setAux("-1");
        for (Value x : listaPreOrdem) {
            if ((!x.getPackagi().equalsIgnoreCase(auxv.getPackagi()) && !auxv.getPackagi().equalsIgnoreCase("-1"))
    ...
    public StringBuffer geraTabelaMethod(ArrayList<Metrics> listMetrics) {
        ...
        String auxm = "-1";
        String auxc = "-1";
        int contNome = 0, contGeral = 0, tamNomes = nomesMetricas.size();
        for (Value x : agrupado) {
            if (contGeral != 0) {
                if ((!x.getPackagi().equalsIgnoreCase(auxp) || !x.getName().equalsIgnoreCase(auxm) || !x.getSource().equalsIgnoreCase(auxc))) {//quando muda de linha
                    saida.append("\n" + x.getPackagi() + ":" + x.getSource() + ":" + x.getName());
                    contGeral++;
    ...
    private ArrayList<String> getNomesMetricasFilter(ArrayList<Metrics> listMetrics) {
        ...
        ArrayList<String> nomesAux = new ArrayList<String>();
        String nomeAux = "-1";
