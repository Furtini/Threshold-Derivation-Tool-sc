...
public class GenerateTablePerType {
    ...
    private ArrayList<Integer> getQtdClassesRepetidas() {
        ...
        for (Value x : listaPreOrdem) {
            if (!(x.getPackagi() + ":" + x.getName()).equalsIgnoreCase(auxc) && !auxc.equalsIgnoreCase("-1")) {
    ...
    public StringBuffer geraTabelaType(ArrayList<Metrics> listMetrics) {
        ...
        for (Value x : agrupado) {
            ...
            if (!x.getPackagi().equalsIgnoreCase(auxp) || !x.getName().equalsIgnoreCase(auxc)) {//quando muda de linha
                saida.append("\n" + x.getPackagi() + ":" + x.getName());
                //saida += "," + x.getValue();
                contGeral++;
